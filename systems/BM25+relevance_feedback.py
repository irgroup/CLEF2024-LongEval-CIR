...
def get_relevance_feedback_topics(topics, history, conn):
    ...
    for _, topic in topics.iterrows():
        new_topic = {
            "qid": topic["qid"],
            "query": topic["query"],
            "query_original": topic["query"],
        }

        # get queryies for potential extension
        queryies_to_extend = query_map[query_map["qid_1"] == topic["qid"]][
            "qid_2"
        ].tolist()
...
def main(args):
    ...
    pt.io.write_results(
        merged_run,
        results_path + f"/CIR_BM25_D-{args.index}_T-{topics_name}_rf{''.join(history)}",
    )
